...
class Drive:
	def __init__(self):
		...
		self.GOT_THE_BALL = False
		
		#SUBSCRIBERS
		self.img_sub = rospy.Subscriber("/ball_location_center",Point, self.drive_callback)
		#self.imu_sub = rospy.Subscriber("/imu",Imu, self.getRotation)
	...
	def drive_callback(self, data):
		...
		print(self.redline_y,self.ballbot_y)
		...
		if not self.BALL_FLAG:
			if ball_x <0 and ball_y <0 and self.ballbot_y == 0 :
				...
				elif abs(norm_x) < 0.15 and self.ballbot_y >= 410: 	#GONNA HOLD THE BALL
